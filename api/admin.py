...
class DocumentAdmin(admin.ModelAdmin):
    ...
    def export_as_csv(self, request, queryset):
        ...
        writer = csv.writer(response)
        writer.writerow(['Document Name', 'Content', 'Created At', 'Identifier'])
    ...
    def export_as_xls(self, request, queryset):
        ...
        ws = wb.add_sheet('Projects')
        row_num = 0
        columns = ['Document Name', 'Content', 'Created At', 'Identifier']
        for col_num, column_title in enumerate(columns):
            ws.write(row_num, col_num, column_title)
        for document in queryset:
            row_num += 1
            row = [document.document_name, document.content, document.identifier, document.created_at]
            for col_num, cell_value in enumerate(row):
                ws.write(row_num, col_num, cell_value)
        wb.save(response)
        return response
